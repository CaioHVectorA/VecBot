...
export const pokeData = Object.values(data).filter((item) => {
    // @ts-ignore
    return !!(item.name && item.types && item.forme !== 'Gmax' && 
    item.num && item.baseStats && item.isNonstandard !== 'CAP') &&
    //tirando skins do pikachu
    !item.name.startsWith('Pikachu-') && !item.name.startsWith('Pichu-')
    //tirando esses coisos inuteis de in-game :P
     && !item.name.includes('Totem') && !item.name.includes('Paldea')
     // zen, esse treco do basculin
     && !item.name.includes('Striped') && !item.name.includes('Galar-Zen')
     // formas do cramorant, toxictry
     && !item.name.startsWith('Cramorant-') && !item.name.startsWith('Toxtricity-')
  }) as NatureType[]